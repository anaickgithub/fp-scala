...
abstract class TweetSet {
  ...
  def ascendingByRetweet: Trending = {
    ...
    if (this.isEmpty) EmptyTrending;
    else new NonEmptyTrending(this.findMin, remove(this.findMin).ascendingByRetweet)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    // if (p(elem)) left.filter0(p, right.filter0(p, acc.incl(elem)))
